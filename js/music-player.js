(function () {
  ...
  root.innerHTML = '<div class="music-particles" aria-hidden="true"></div><button class="music-star" type="button" aria-label="展开音乐播放器" aria-expanded="false"><span>✦</span></button><div class="music-panel" aria-hidden="true"><div class="music-orbit" aria-hidden="true"><i></i><i></i><i></i></div><div class="music-copy"><strong class="music-title"></strong><span class="music-artist"></span></div><div class="music-controls"><button class="music-prev" type="button" aria-label="上一首">‹</button><button class="music-play" type="button" aria-label="播放">▶</button><button class="music-next" type="button" aria-label="下一首">›</button></div><div class="music-progress"><span></span></div></div>';
  ...
  star.addEventListener("click", function () { setExpanded(!root.classList.contains("expanded")); }); play.addEventListener("click", togglePlayback);
